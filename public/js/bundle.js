...
var showAlert = function showAlert(type, msg) {
  ...
  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); // remove once the alert is shown
...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    ...
    scrollZoom: false // first longitude and then latitude
  ...
  var bounds = new mapboxgl.LngLatBounds(); //2 loop all the locations array and add marker for each of them
  ...
  locations.forEach(function (location) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(location.coordinates).addTo(map); //3 Add popup

    new mapboxgl.Popup({
      offset: 30 // the popup overlaps with marker, so set offset

    }).setLngLat(location.coordinates).setHTML("<p>Day ".concat(location.day, ": ").concat(location.description, "</p>")).addTo(map); //4 Extend map bounds to include current location

    bounds.extend(location.coordinates); // map.fitBounds(bounds)
...
var stripe = Stripe('pk_test_51Kt9XQSChdbo5Stnd5mTHloYr1Xtim7RVYBCsaJ9x8EpmifsgGarVzhmvTQ7TgmhDEOfE2VNDrQpf31Yb2p2zVSP001evS55PI');
console.log(stripe.redirectToCheckout); // This is the function will be executed, when we click the bookTour button
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            resSession = _context.sent;
            console.log(resSession); // 2) Create checkout form + chanre credit card
...
if (loginForm) {
  loginForm.addEventListener('submit', function (event) {
    ...
    console.log('loginform'); // Get the values

    var email = document.getElementById('email').value;
    var password = document.getElementById('password').value; // Make the Ajax call
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            passwordConfirm = document.getElementById('password-confirm').value; // we are awaiting here, so once it is finished, we can do some other work below, i,e clear the password fields
...
if (bookBtn) bookBtn.addEventListener('click', function (e) {
  ...
  e.target.textContent = 'Processing...'; // data-tour-id=`${tour.id}` Here tour-id when ever there is a dash it will automatically get converted to camelCase --> tourId
